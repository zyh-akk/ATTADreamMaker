    ...
    <div class="dream-maker-model" v-show="nftsBol">
      ...
      <div class="nft-list">
        <div v-for="(item,index) in nftlist" :key="index" class="dream-maker-nft">
          ...
          <span v-if="item.creator_tag == 1">User xxx created for you</span>
          ...
          <button v-if="item.status == 0 && item.type == 2 && item.creator_tag == 1">Accept</button>
          <button v-if="item.status == 1 && item.type == 2 && item.creator_tag == 1">Mint now</button>
        </div>
      </div>
    </div>
  </div>
